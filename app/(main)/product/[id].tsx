...
export default function ProductDetails() {
    ...
    useEffect(() => {
        ...
        fetch("https://dummyjson.com/products/" + id).then(data => data.json()).then((data) => { setproduct(data) })
    ...
    /* ---------------- UI ---------------- */
const disprice= Math.round( product.discountPercentage>0 ? (product.price -(product.price * product.discountPercentage) / 100) : product.price);
    ...
    return (
        <View className="flex-1 bg-white">
            ...
            <Animated.View
                style={headerStyle}
                className="absolute top-0 left-0 right-0 z-20 bg-white pt-6 pb-7 px-4 pb-3 shadow shadow-[0px_0px_16px_5px_gray]"
            >
                ...
                <View className="overflow-hidden">
                    ...
                    <View className="absolute bottom-6 self-center flex-row">
                        {product.images.map((_, i) => (
                            <View
                                key={i}
                                className={`h-2 w-4 mx-1 rounded-full`} style={{ backgroundColor: i === activeIndex ? Colors.primary : '#C2C2C2' }}
                            />
                        ))}

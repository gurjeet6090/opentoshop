...
const filterConfig = {
  filters: [
    {
      ...
      type: "checkbox",
      items: [
        { id: 1, name: "Programming", count: 125 },
        { id: 2, name: "Business", count: 65 },
        { id: 3, name: "Science", count: 45 },
        { id: 4, name: "History", count: 21 },
        { id: 5, name: "Kids", count: 98 },
      ],
    },
    {
      id: "price",
  ...
  sort: [
    { id: "menu_order", label: "Default Sorting" },
...
const CustomRangeSlider = ({
  ...
  const tempMinRef = useRef(value.min);
  const tempMaxRef = useRef(value.max);

  const minStart = useRef(0);
  const maxStart = useRef(0);

  const sliderWidth = Dimensions.get("window").width - 60; // Account for padding
  ...
  const getValueFromPosition = (position: number) => {
    return Math.round((position / sliderWidth) * (max - min) + min);
  };

  // Create PanResponder for min thumb
  ...
  const minPanResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: () => {
        minStart.current = getPositionFromValue(tempMinRef.current);
      },
      onPanResponderMove: (evt, gestureState) => {
        let newX = minStart.current + gestureState.dx;

        newX = Math.max(0, Math.min(newX, getPositionFromValue(tempMax) - 20));
        const newVal = getValueFromPosition(newX);
        if (newVal < tempMaxRef.current) {
          tempMinRef.current = newVal;
          setTempMin(newVal);
        }
      },
      onPanResponderRelease: () => {
        onValueChange({
          min: tempMinRef.current,
          max: tempMaxRef.current,
        });
      },
    }),
  ).current;

  // Create PanResponder for max thumb
  ...
  const maxPanResponder = useRef(
    PanResponder.create({
      ...
      onMoveShouldSetPanResponder: () => true,
      onPanResponderGrant: () => {
        maxStart.current = getPositionFromValue(tempMaxRef.current);
      },
      onPanResponderMove: (evt, gestureState) => {
        let newX = maxStart.current + gestureState.dx;

        newX = Math.min(
          sliderWidth,
          Math.max(newX, getPositionFromValue(tempMin) + 20),
        );
        const newVal = getValueFromPosition(newX);
        if (newVal > tempMinRef.current) {
          tempMaxRef.current = newVal;
          setTempMax(newVal);
        }
      },
      onPanResponderRelease: () => {
  ...
  const handleMinChange = (text: string) => {
    ...
    const newMin = Math.min(Math.max(val, min), tempMax - 1);
    setTempMin(newMin);

    onValueChange({ min: newMin, max: tempMax });
  };

  const handleMaxChange = (text: string) => {
    const val = parseInt(text) || 0;
    const newMax = Math.max(Math.min(val, max), tempMin + 1);
    setTempMax(newMax);

    onValueChange({ min: tempMin, max: newMax });
  };
  ...
  useEffect(() => {
    ...
    tempMinRef.current = value.min;
    tempMaxRef.current = value.max;
  }, [value]);
  return (
    <View className="px-4">
      {/* Slider Track */}
      <View className="relative h-8 justify-center mt-2 ">
        {/* Background Track */}
        <View className="absolute h-1 w-full bg-gray-300 rounded-full " />

        {/* Selected Range */}
        <View
          className="absolute h-1 bg-primary rounded-full"
          style={{
            left: getPositionFromValue(tempMin),
            width:
              getPositionFromValue(tempMax) - getPositionFromValue(tempMin),
        ...
        <Animated.View
          ...
          className="absolute w-6 h-6 bg-white border-2 border-primary rounded-full justify-center items-center"
          style={{
            left: getPositionFromValue(tempMin) - 12,
            top: 2,
            shadowColor: "#000",
        ...
        >
          <View className="w-auto absolute left-0 -top-8 bg-gray-800 px-2 py-1 rounded">
            <Text className="text-white text-xs w-full h-[16px] ">
              ${tempMin}
            </Text>
          </View>
        ...
        <Animated.View
          ...
          className="absolute w-6 h-6 bg-white border-2 border-primary rounded-full justify-center items-center"
          style={{
            left: getPositionFromValue(tempMax) - 12,
            top: 2,
            shadowColor: "#000",
            shadowOffset: { width: 0, height: 2 },
            shadowOpacity: 0.3,
        ...
        >
          <View className="absolute w-auto right-0 -top-8 bg-gray-800 px-2 py-1 rounded">
            <Text className="text-white text-xs w-full h-[16px]">
              ${tempMax}
            </Text>
          </View>
...
export default function Shop() {
  ...
  const getFilteredProducts = useCallback(() => {
    ...
    if (filters.category.length > 0) {
      // Map category IDs to actual category names (since your products use string categories)
      ...
      const selectedCategories = filters.category.map((id) =>
    ...
    if (filters.stock.includes("onsale")) {
    ...
    switch (selectedSort.id) {
      ...
      case "rating":
        ...
        break;
      case "latest":
        // Sort by ID as a proxy for latest (higher ID = newer)
        filtered.sort((a, b) => b.id - a.id);
        break;
      case "popularity":
        // Sort by stock as a proxy for popularity (lower stock = more popular)
  ...
  const getActiveFilterCount = useCallback(() => {
    ...
    if (filters.stock.length > 0) count++;
    if (
      filters.price.min > 0 ||
      filters.price.max < 5000 ||
      (filters.price.min !== 0 && filters.price.max !== 5000)
    )
      count++;
    return count;
  }, [filters]);

  // Handle checkbox selection with proper type checking
  const handleCheckboxToggle = (
    filterId: keyof FilterState,
    itemId: string | number,
  ) => {
    setFilters((prev) => {
      const current = prev[filterId] as (string | number)[];
      if (Array.isArray(current)) {
        const updated = current.includes(itemId)
          ? current.filter((id) => id !== itemId)
          : [...current, itemId];
        return { ...prev, [filterId]: updated as any };
      }
      return prev;
    });
  };
  ...
  const handleRadioSelect = (filterId: keyof FilterState, value: number) => {
    setFilters((prev) => ({ ...prev, [filterId]: value as any }));
  };
        ...
        return (
          <View key={filter.id} className="mb-6">
            ...
            {filter.items.map((item: any) => {
              const filterKey = filter.id as keyof FilterState;
              const currentValue = filters[filterKey] as (string | number)[];

              const isSelected =
                Array.isArray(currentValue) &&
                currentValue.includes(item.id || item.value);
              return (
                <TouchableOpacity
                  key={item.id || item.value}
                  className="flex-row items-center py-2.5"
                  onPress={() =>
                    handleCheckboxToggle(filterKey, item.id || item.value)
                  }
                >
                  <View
      ...
      case "radio":
        return (
          <View key={filter.id} className="mb-6">
            ...
            {filter.items.map((item: any) => {
              const filterKey = filter.id as keyof FilterState;
              const isSelected = filters[filterKey] === item.value;
              return (
                <TouchableOpacity
                  key={item.value}
                  className="flex-row items-center py-2.5"
                  onPress={() => handleRadioSelect(filterKey, item.value)}
                >
                  <View
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filters.rating !== null) count++;
    if (filters.stock.length > 0) count++;
    if (filters.price.min > 0 || filters.price.max < 5000) count++;
    return count;
  }, [filters]);
    ...
    <View className="flex-1 bg-gray-100">
      ...
      >
        <View className="flex-row justify-between items-center w-full gap-4">
          ...
          <Text className="text-2xl font-bold">Shop</Text>
      ...
      <View className="absolute bottom-[60px] left-0 right-0 z-10 flex-row justify-center px-4">
        <TouchableOpacity
          className="flex-row items-center shadow-xl rounded-full px-4 py-2 "
          onPress={() => setFilterVisible(true)}
          style={{
            backgroundColor: Colors.primary,
          }}
        >
          <Text className="text-white text-lg font-semibold mr-2">Filter</Text>
          <Ionicons name="options-outline" size={22} color="white" />
          {getActiveFilterCount() > 0 && (
            <View className="absolute shadow-xl -top-1 -right-1 bg-white rounded-full min-w-[20px] h-5 flex items-center justify-center px-1">
        ...
        <TouchableOpacity
          ...
          className="flex-row items-center shadow-xl rounded-full px-4 py-2 mx-1 "
          style={{
            backgroundColor: Colors.primary,
          }}
        >
          <Text className="text-white text-lg font-semibold mr-2">Sort</Text>
      ...
      >
        ...
        >
          <Pressable
            className="bg-white rounded-t-3xl p-5 max-h-[85vh]"
          ...
          >
            ...
            <View className="flex-row mt-4 pt-3 border-t border-gray-100 gap-3">
              <TouchableOpacity
              ...
              <TouchableOpacity
                className="flex-1 rounded-xl py-3 items-center shadow-sm active:opacity-70"
                style={{ backgroundColor: Colors.primary }}
      ...
      >
        ...
        >
          <Pressable
            className="bg-white rounded-t-3xl p-5 max-h-[60vh]"
            onPress={(e) => e.stopPropagation()}
          >
            <View className="flex-row justify-between items-center mb-5 pb-3 border-b border-gray-100">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {filterConfig.sort.map((option) => {
                const isSelected = selectedSort.id === option.id;
                return (
                  <TouchableOpacity
                    key={option.id}
                    className="flex-row justify-between items-center py-4 border-b border-gray-50 active:opacity-70"
                    onPress={() => {
                  ...
                  >
                    <Text
                      className={`text-base ${isSelected ? "font-semibold" : "text-gray-600"}`}
                      style={{ color: isSelected ? Colors.primary : "#4B5563" }}
                    >
                      {option.label}
                    </Text>
                    {isSelected && (

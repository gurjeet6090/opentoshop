...
export default function SearchComponent({
  ...
  const translateYAnim = useMemo(() => new Animated.Value(50), []);
  ...
  const results = useMemo(() => {
    ...
    const categoryResults = categories
      .filter((cat: any) => cat.toLowerCase().includes(query))
      .map((cat) => ({
        id: `cat-${cat}`,
        title: cat,
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              className="flex-row items-center py-3 px-2 active:opacity-70 border-b border-gray-50"
            >
    ...
    <FlatList
      ...
      renderItem={({ item }) =>
        item.type === "category" ? (
          ...
          <TouchableOpacity
            className="flex-row py-3 px-2 active:opacity-70 border-b border-gray-50"
            onPress={() => handleProductPress(item as Product)}
          >
            <Image
    ...
    >
      <View className="flex-1 px-4 pt-6">
        ...
        <View className="flex-row items-center mb-4">
          ...
          <View
            className={`flex-1 flex-row items-center ml-2 rounded-2xl border ${
              isFocused || search
                ? "border-indigo-400 bg-indigo-50/30"
                : "border-gray-200 bg-gray-50"
            }`}
            style={{
              borderColor: isFocused || search ? "#a8a8a8" : "#E5E7EB",
            }}
          >
            <Ionicons
              name="search-outline"
            ...
            <TextInput
              ...
              className="flex-1 h-12 px-3 text-base text-gray-800"
              autoFocus
        ...
        {/* Search Body */}
        {!search ? renderRecentSearches() : renderSearchResults()}
      </View>
    </Animated.View>
  );
}
